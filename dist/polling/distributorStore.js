...
// ---- Public API ----
export async function addDistributor(chainKey, address) {
...
export async function touchDistributor(chainKey, address) {
    const addr = address.toLowerCase();
    try {
        const { error } = await getSupabase()
            .from('tracked_distributors')
...
export async function getDistributors(chainKey) {
    await ensureCache();
...
export async function totalTracked() {
    await ensureCache();
    let total = 0;
...
export async function initStore() {
    await refreshCache();
    setInterval(() => {
        cleanupExpired().catch((err) => console.error('[distributorStore] scheduled cleanup error:', err?.message));
    }, 60 * 60 * 1000);
}
